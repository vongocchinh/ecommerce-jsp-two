...
public ArrayList<Product> getRanDom() throws SQLException {
    Connection connection = DBConnect.getConnecttion();
    String sql = "SELECT * FROM product ORDER BY RAND() LIMIT 5 ";
...
public ArrayList<Product> getListProductByBrand_Lienquan(int category_ID,int brand_id) throws SQLException {
    Connection connection = DBConnect.getConnecttion();
    String sql = "SELECT * FROM product WHERE brand_id = '" + brand_id + "' and category_id = '" + category_ID + "' ORDER BY RAND() LIMIT 5";
    PreparedStatement ps = connection.prepareCall(sql);
    ResultSet rs = ps.executeQuery();
    ArrayList<Product> list = new ArrayList<>();
    while (rs.next()) {
